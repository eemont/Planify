...
import 'dart:html' as html; // For file download
...
class MutualAvailabilityPage extends StatelessWidget {
  final List<Map<String, dynamic>> allAvailability; // Includes availability, colors, and names
  final List<String> days; // Days on the X-axis
  final List<String> hours; // Hours on the Y-axis

  final GlobalKey repaintBoundaryKey = GlobalKey(); // Key for capturing the widget

  MutualAvailabilityPage({
    required this.allAvailability,
    required this.days,
    required this.hours,
    super.key,
  ...
  List<List<Set<int>>> calculateSlotSelections() {
    ...
    for (int personIndex = 0; personIndex < allAvailability.length; personIndex++) {
      final availability =
  ...
  // Function to capture the schedule as a JPEG image
  Future<void> downloadJPEG() async {
    try {
      ...
      if (byteData != null) {
        ...
        // Use html package to trigger the download
        final blob = html.Blob([buffer]);
        final url = html.Url.createObjectUrlFromBlob(blob);
        final anchor = html.AnchorElement(href: url)
          ..target = 'blank'
          ..download = 'mutual_availability.jpeg'
          ..click();
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Wrap the schedule grid and legend with RepaintBoundary
            Expanded(
                      ...
                      SizedBox(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final name = allAvailability[index]['name'] as String;
                            return Padding(
